...
def setup_mcp_server(hexstrike_client: HexStrikeClient) -> FastMCP:
    ...
    @mcp.tool()
    def cent_sync(path: str = "", console: bool = False, threads: int | None = None) -> Dict[str, Any]:
        data = {"path": path, "console": console}
        if threads is not None:
            data["threads"] = threads
        return hexstrike_client.safe_post("api/tools/cent_sync", data)

    @mcp.tool()
    def cent_update(path: str = "", delete_dirs: bool = True, delete_files: bool = False,
                    console: bool = False, threads: int | None = None) -> Dict[str, Any]:
        data = {"path": path, "delete_dirs": delete_dirs, "delete_files": delete_files, "console": console}
        if threads is not None:
            data["threads"] = threads
        return hexstrike_client.safe_post("api/tools/cent_update", data)

    @mcp.tool()
    def cent_summary(path: str = "", json_output: bool = True) -> Dict[str, Any]:
        data = {"path": path, "json": json_output}
        return hexstrike_client.safe_post("api/tools/cent_summary", data)

    @mcp.tool()
    def lfi_quick(host: str = "", urls_file: str = "", payload: str = "/etc/passwd",
                  concurrency: int = 25, additional_args: str = "", use_recovery: bool = True) -> Dict[str, Any]:
        data = {"host": host, "urls_file": urls_file, "payload": payload,
                "concurrency": concurrency, "additional_args": additional_args, "use_recovery": use_recovery}
        return hexstrike_client.safe_post("api/tools/lfi_quick", data)

    @mcp.tool()
    def redirect_hunter(host: str = "", lhost: str = "", concurrency: int = 25,
                        additional_args: str = "", use_recovery: bool = True) -> Dict[str, Any]:
        data = {"host": host, "lhost": lhost, "concurrency": concurrency,
                "additional_args": additional_args, "use_recovery": use_recovery}
        return hexstrike_client.safe_post("api/tools/redirect_hunter", data)

    @mcp.tool()
    def redirect_burp_parallel(urls_file: str = "", jobs: int = 10, proxy: str = "http://127.0.0.1:8080",
                               out_file: str = "url-redirect.txt", additional_args: str = "", use_recovery: bool = True) -> Dict[str, Any]:
        data = {"urls_file": urls_file, "jobs": jobs, "proxy": proxy,
                "out_file": out_file, "additional_args": additional_args, "use_recovery": use_recovery}
        return hexstrike_client.safe_post("api/tools/redirect_burp_parallel", data)

    @mcp.tool()
    def xss_spider_dalfox(urls_file: str = "", concurrency: int = 10, depth: int = 5,
                          blacklist: str = r".(jpg|jpeg|gif|css|tif|tiff|png|ttf|woff|woff2|ico|pdf|svg|txt)",
                          out_file: str = "xss-dalfox.txt", additional_args: str = "", use_recovery: bool = True) -> Dict[str, Any]:
        data = {"urls_file": urls_file, "concurrency": concurrency, "depth": depth,
                "blacklist": blacklist, "out_file": out_file, "additional_args": additional_args, "use_recovery": use_recovery}
        return hexstrike_client.safe_post("api/tools/xss_spider_dalfox", data)

    @mcp.tool()
    def xss_wayback_dalfox(host: str = "", blind: str = "", file_tmp: str = "xss-params.txt",
                           out_file: str = "xss-dalfox-blind.txt", additional_args: str = "", use_recovery: bool = True) -> Dict[str, Any]:
        data = {"host": host, "blind": blind, "file_tmp": file_tmp,
                "out_file": out_file, "additional_args": additional_args, "use_recovery": use_recovery}
        return hexstrike_client.safe_post("api/tools/xss_wayback_dalfox", data)

    @mcp.tool()
    def js_message_listener_scan(hosts_file: str = "", additional_args: str = "", use_recovery: bool = True) -> Dict[str, Any]:
        data = {"hosts_file": hosts_file, "additional_args": additional_args, "use_recovery": use_recovery}
        return hexstrike_client.safe_post("api/tools/js_message_listener_scan", data)

    @mcp.tool()
    def proto_pollution_quickcheck(host: str = "", threads: int = 300,
                                   alive_file: str = "alive.txt", additional_args: str = "", use_recovery: bool = True) -> Dict[str, Any]:
        data = {"host": host, "threads": threads, "alive_file": alive_file,
                "additional_args": additional_args, "use_recovery": use_recovery}
        return hexstrike_client.safe_post("api/tools/proto_pollution_quickcheck", data)

    @mcp.tool()
    def cve_2020_5902_check(query: str = 'http.favicon.hash:-335242539 "3992"', insecure: bool = True,
                            use_recovery: bool = True) -> Dict[str, Any]:
        data = {"query": query, "insecure": insecure, "use_recovery": use_recovery}
        return hexstrike_client.safe_post("api/tools/cve_2020_5902_check", data)

    @mcp.tool()
    def cve_2020_3452_check(hosts_file: str = "", use_recovery: bool = True) -> Dict[str, Any]:
        data = {"hosts_file": hosts_file, "use_recovery": use_recovery}
        return hexstrike_client.safe_post("api/tools/cve_2020_3452_check", data)

    @mcp.tool()
    def cve_2022_0378_check(urls_file: str = "", use_recovery: bool = True) -> Dict[str, Any]:
        data = {"urls_file": urls_file, "use_recovery": use_recovery}
        return hexstrike_client.safe_post("api/tools/cve_2022_0378_check", data)

    @mcp.tool()
    def vbulletin_562_rce_check(query: str = 'http.favicon.hash:-601665621', use_recovery: bool = True) -> Dict[str, Any]:
        data = {"query": query, "use_recovery": use_recovery}
        return hexstrike_client.safe_post("api/tools/vbulletin_562_rce_check", data)

    @mcp.tool()
    def asn_routes_by_ip(ip: str = "", use_recovery: bool = True) -> Dict[str, Any]:
        data = {"ip": ip, "use_recovery": use_recovery}
        return hexstrike_client.safe_post("api/tools/asn_routes_by_ip", data)

    @mcp.tool()
    def extract_ips_from_file(file_path: str = "", out_file: str = "", use_recovery: bool = True) -> Dict[str, Any]:
        data = {"file_path": file_path, "out_file": out_file, "use_recovery": use_recovery}
        return hexstrike_client.safe_post("api/tools/extract_ips_from_file", data)
    ...
    def naabu_fast_scan(
        ...
        exclude_ports: str = "",
        rate: int = 1000,
        scan_type: str = "s",           # "s" (SYN) atau "c" (CONNECT)
        retries: int = 3,
        timeout: int = 1000,            # ms
        verify: bool = False,
        ping: bool = False,
        ...
        """
        Jalankan Naabu (fast port scanner) dengan logging dan error handling konsisten.

        Args:
            target: satu host/hostname/IP (opsional kalau pakai 'hosts' atau 'list_file')
            hosts: list beberapa host untuk discan (dipetakan ke -host comma-separated)
            list_file: path file berisi daftar host (-l)
            ports: daftar port (mis. "80,443,100-200") (-p)
            top_ports: preset top ports (mis. "100" / "1000" / "full") (-tp)
            exclude_ports: port yang dikecualikan (-ep)
            rate: PPS (-rate)
            scan_type: "s" untuk SYN (default), "c" untuk CONNECT (-s)
            retries: jumlah retry (-retries)
            timeout: timeout per port dalam ms (-timeout)
            verify: validasi port ulang TCP (-verify)
            ping: ping probes (-ping)
            exclude_cdn: skip full scan untuk CDN/WAF (-ec)
            interface: interface jaringan (-i)
            source_ip: source ip:port (-source-ip)
            passive: tampilkan passive open ports (Shodan InternetDB) (-passive)
            nmap_cli: jalankan nmap command pada hasil (-nmap-cli "nmap -sV")
            json_output: output JSON lines (-json)
            additional_args: argumen tambahan mentah
            use_recovery: pakai execute_command_with_recovery di server
        """
        data = {
            "target": target,
            "hosts": hosts or [],
        ...
        logger.info(f"⚡ Starting Naabu: target={target or hosts or list_file} rate={rate}")
        result = hexstrike_client.safe_post("api/tools/naabu", data)
        ...
        if result.get("success"):
            ...
            if result.get("recovery_info", {}).get("recovery_applied"):
                ...
                logger.info(f"🛠️ Recovery applied during Naabu: attempts={attempts}")
        else:
            logger.error("❌ Naabu scan failed")
    ...
    def nuclei_scan(
        ...
        template: str = "",
        additional_args: str = "",
        cent_update: bool = False,          # <-- minta server jalankan `cent update` dulu
        use_recovery: bool = True           # <-- default pakai recovery server
    ) -> Dict[str, Any]:
        """
        Execute Nuclei vulnerability scanner with enhanced logging and real-time progress.

        Args:
            target: Single target URL/IP/host (maps to `-u`). Kosongkan jika pakai urls_file.
            urls_file: File berisi banyak target (maps to `-l`).
            severity: Filter severity, e.g. "critical,high,medium".
            tags: Filter tags, e.g. "cve,rce,lfi". (Kosongkan untuk semua template).
            template: Path template kustom; jika kosong, server pakai NUCLEI_TEMPLATES_PATH (cent).
            additional_args: Argumen tambahan mentah untuk Nuclei.
            cent_update: Jika True, server jalankan `cent update -p <path> -d` sebelum scan.
            use_recovery: Gunakan intelligent recovery di server.

        Returns:
            JSON hasil eksekusi (stdout/stderr, success flag, telemetry, recovery_info).
        """
        if not target and not urls_file:
            # selaras dengan validasi di server: salah satu harus ada
            raise ValueError("Provide 'target' or 'urls_file' for nuclei_scan")
        ...
        data = {
            ...
            "severity": severity,
            "tags": tags,
            "template": template,           # biarkan "" agar server pakai default cent
            "additional_args": additional_args,
            "cent_update": cent_update,
            "use_recovery": use_recovery,
        }

        shown_target = target or urls_file
        logger.info(f"{HexStrikeColors.BLOOD_RED}🔬 Starting Nuclei vulnerability scan: {shown_target}{HexStrikeColors.RESET}")

        result = hexstrike_client.safe_post("api/tools/nuclei", data)

        if result.get("success"):
            logger.info(f"{HexStrikeColors.SUCCESS}✅ Nuclei scan completed for {shown_target}{HexStrikeColors.RESET}")

            # Optional: highlight cepat berdasarkan output
            ...
            rec = result.get("recovery_info", {})
            if rec.get("recovery_applied"):
                attempts = rec.get("attempts_made", 1)
                logger.info(f"{HexStrikeColors.HIGHLIGHT_YELLOW} Recovery applied: {attempts} attempts made {HexStrikeColors.RESET}")
        else:
            logger.error(f"{HexStrikeColors.ERROR}❌ Nuclei scan failed for {shown_target}{HexStrikeColors.RESET}")
